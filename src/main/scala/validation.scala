...
object valiation {
  ...
  object InvalidFields {
    case class InvalidEmail(override val errors: List[String], override val field: String = "email") extends InvalidField
    case class InvalidPassword(override val errors: List[String], override val field: String = "password") extends InvalidField
    case class InvalidUsername(override val errors: List[String], override val field: String = "username") extends InvalidField
    case class InvalidTitle(override val errors: List[String], override val field: String = "title") extends InvalidField
    case class InvalidDescription(override val errors: List[String], override val field: String = "description") extends InvalidField
    case class InvalidBody(override val errors: List[String], override val field: String = "body") extends InvalidField

    case class InvalidEmailOrPassword(override val errors: List[String], override val field: String = "email or password") extends InvalidField
  }
  ...
  def validPassword(password: String): ValidationResult[String] =
    (notBlank(password), min(password, 8), max(password, 100)).mapN({ case t => t._1 }).leftMap(toInvalidField(_, InvalidPassword.apply(_)))

  def validUsername(username: String): ValidationResult[String] = {
    val trimmedUsername = username.trim
    (notBlank(trimmedUsername), min(trimmedUsername, 1), max(trimmedUsername, 25)).mapN({ case t => t._1 }).leftMap(toInvalidField(_, InvalidUsername.apply(_)))
  }
  ...
  def validDescription(description: String): ValidationResult[String] = {
    ...
    notBlank(trimmedDescription).leftMap(toInvalidField(_, InvalidDescription.apply(_)))
  }
  ...
  def toInvalidField[F <: InvalidField](nec: NonEmptyChain[String], mkInvalidField: List[String] => F): NonEmptyChain[F] =
  ...
  object validators {
    ...
    def min(s: String, minSize: Int): ValidatorResult[String] =
      if (s.size >= minSize) s.validNec else s"is too short (minimum is $minSize character)".invalidNec

    def max(s: String, maxSize: Int): ValidatorResult[String] =
      if (s.size <= maxSize) s.validNec else s"is too long (maximum is $maxSize character)".invalidNec
