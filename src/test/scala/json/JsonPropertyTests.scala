...
object JsonPropertyTests extends Properties("JsonSchema") {
  ...
  def resizeGen[A](f: Int => Int, gen: Gen[A]): Gen[A] = Gen.sized(i => Gen.resize(f(i), gen))
  ...
  def convertTree[A, B]: Tree[A] => JsonTree[A] = {
    case Tree.Node(left, right) => Fix(Left(JsonObject(("left", convertTree(left)), ("right", convertTree(right)))))
